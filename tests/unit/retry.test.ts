...
test("shouldSkip returns true — throws SKIP", async () => {
  const fn = mock(() => Promise.resolve("success"));
  const shouldSkip = mock(() => Promise.resolve(true));

  await expect(
    withRetry(fn, { shouldSkip, maxAttempts: 3 })
  ).rejects.toThrow("SKIP_ALREADY_PROCESSED");
...
test("does not retry on SKIP error from function", async () => {
  const fn = mock(() => {
    const err: any = new Error("SKIP_ALREADY_PROCESSED");
    err.code = "SKIP";
    return Promise.reject(err);
  });

  await expect(withRetry(fn, { maxAttempts: 3 })).rejects.toThrow("SKIP_ALREADY_PROCESSED");
  expect(fn.mock.calls.length).toBe(1);
});
