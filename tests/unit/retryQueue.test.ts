...
test("receives RetryCommand, re-publishes FormUploaded if form was submitted", async () => {
  ...
  const formEvents = published.filter((e: any) => e.type === EventType.init_quote_form_upload);
...
test("re-publishes FileUploaded for parts with 2D and 3D keys", async () => {
  ...
  const published = eventBus.getPublished();
  const fileEvents = published.filter((e: any) => e.type === EventType.init_quote_part_2d_file_upload || e.type === EventType.init_quote_part_3d_file_upload);
  expect(fileEvents.length).toBe(3);

  const p1_2d = fileEvents.find((e: any) => e.payload.partId === "p1" && e.type === EventType.init_quote_part_2d_file_upload);
  expect(p1_2d).toBeTruthy();
  const p1_3d = fileEvents.find((e: any) => e.payload.partId === "p1" && e.type === EventType.init_quote_part_3d_file_upload);
  expect(p1_3d).toBeTruthy();
  const p2_3d = fileEvents.find((e: any) => e.payload.partId === "p2" && e.type === EventType.init_quote_part_3d_file_upload);
...
test("returns early if quote not found", async () => {
  ...
  const retryEvents = published.filter(
    (e: any) => e.type === EventType.init_quote_form_upload || e.type === EventType.init_quote_part_2d_file_upload || e.type === EventType.init_quote_part_3d_file_upload
  );
