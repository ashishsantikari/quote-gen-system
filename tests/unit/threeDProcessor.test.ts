...
test("subscribes to QuoteRequestReceived and processes 3D files for all parts", async () => {
  const dataService = createMockDataService({
    getQuote: mock(() => Promise.resolve({
      quoteId: "q1",
      parts: [{ partId: "p1", name: "engine", file3DKey: "quotes/q1/parts/p1/3d/model.stl" }],
    })),
  });
  ...
  const processedEvents = published.filter((e: any) => e.type === EventType.part_3d_complete);
...
test("idempotency check skips already processed 3D parts", async () => {
  ...
  const successEvents = published.filter(
    (e: any) => e.type === EventType.part_3d_complete && e.payload.output !== null
  );
...
test("circuit breaker records failure on file not found", async () => {
  ...
  const failEvents = published.filter(
    (e: any) => e.type === EventType.part_3d_complete && e.payload.error
  );
...
test("skips parts without a file3DKey", async () => {
  const dataService = createMockDataService({
    getQuote: mock(() => Promise.resolve({
      quoteId: "q1",
      parts: [
        { partId: "p1", name: "engine", file3DKey: "quotes/q1/parts/p1/3d/engine.stl" },
