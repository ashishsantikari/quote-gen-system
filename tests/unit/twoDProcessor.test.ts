...
test("subscribes to FileUploaded and processes 2D files", async () => {
  ...
  const processedEvents = published.filter((e: any) => e.type === EventType.Part2DProcessed);
...
test("ignores FileUploaded events that are not 2D", async () => {
  const dataService = createMockDataService();
  const fileStorage = createMockFileStorage();
  const eventBus = createMockEventBus();
...
test("idempotency check skips already processed 2D parts", async () => {
  ...
  const successEvents = published.filter(
    (e: any) => e.type === EventType.Part2DProcessed && e.payload.output !== null
  );
...
test("circuit breaker records failure on file not found", async () => {
  ...
  const failEvents = published.filter(
    (e: any) => e.type === EventType.Part2DProcessed && e.payload.error
  );
