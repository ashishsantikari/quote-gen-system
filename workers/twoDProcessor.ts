...
export function twoDProcessor(
  ...
  eventBus.subscribe(EventType.quote_all_mandatory_data_receipt, async (event) => {
    ...
    log.info("processing 2D files", meta);

    for (const part of quote.parts) {
      const fileKey = part.file2DKey;
      if (!fileKey) continue;
      ...
      await withRetry(
        ...
        log.warn("2D processing failed", { quoteId, partId, fileKey, traceId: event.traceId, error: error.message });
        await eventBus.publish({
          type: EventType.part_2d_complete,
