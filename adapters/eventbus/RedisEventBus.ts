...
export class RedisEventBus implements IEventBus {
  private pub: Redis;
  private sub: Redis;
  private handlers = new Map<string, Set<(event: QuoteEvent) => Promise<void>>>();
  ...
  constructor(redisUrl?: string) {
    ...
    this.pub.on("error", (err) => log.error("pub error", { error: err.message }));
    this.sub.on("error", (err) => log.error("sub error", { error: err.message }));
  }
  ...
  subscribe(eventType: EventType, handler: (event: QuoteEvent) => Promise<void>): void {
    ...
    log.debug("subscribe", { eventType, subscriberCount: this.handlers.get(eventType)!.size });
    ...
    if (this.sub.listenerCount("message") === 0) {
      this.sub.on("message", (channel: string, message: string) => {
        ...
        log.debug("received", { eventType: event.type, traceId: event.traceId });
        for (const h of subs) {
          h(event).catch((err) => log.error("handler error", { eventType: event.type, traceId: event.traceId, error: err.message }));
        }
      });
    }
  }
